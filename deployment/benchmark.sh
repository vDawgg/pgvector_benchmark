...
gcloud compute ssh $CLIENT_INSTANCE_NAME --project $PROJECT_ID --zone $ZONE --tunnel-through-iap --command "sudo sh /pgvector_benchmark/deployment/run_bench_client.sh $SUT_IP $RUN $INDEXING_METHOD"
